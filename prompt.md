...
块 1：{
  ...
  "affiliations": "[作者所属机构/单位]",
  "year": [年份, 数字],
  "venue": "[期刊/会议简称]",
  "category": "[Dataset/Dynamic Secrets/Privacy Policy]",
  "date_read": "年-月-日"
}
块 2：papers_detail.jsonJSON{
  "title": "[标题必须与块 1 完全一致]",
  "summary": "[一句话总结核心贡献/发现]",
  "key_insights": [
    "[关键贡献/发现 1]",
    "[关键贡献/发现 2]",
    "[关键贡献/发现 3]"
...
Role: 你是一位资深的网安隐私领域（Cybersecurity）学者，擅长提取论文核心贡献与实验细节。
Task: 请阅读我提供的论文内容，并严格按照以下两个 JSON 格式输出分析结果。
分类标准 (Category Rules)：Privacy Policy: 隐私协议相关。
输出要求：
1.  严禁捏造事实，所有信息必须基于原文。
2.  Markdown 增强：在 JSON 的字符串内部（尤其是 methodology 和 key_insights）使用 Markdown 语法：
    2.1 使用 **文本** 加粗关键术语。
...
块 2：papers_detail.jsonJSON{
  ...
  "methodology": "[描述其如何将隐私数据分类，分了哪些类型，论文使用隐私数据分类做了什么统计/操作“\n1. **1. xxx**：\n]"
}